...
namespace Zyborg.Vault
{
    ...
    public class DefaultSeal : ISeal
    {
		...
		public byte[][] StoredKeys
		{
			get => throw new InvalidOperationException("core: stored keys are not supported");
			set => throw new InvalidOperationException("core: stored keys are not supported");
        }
        ...
        public SealConfig BarrierConfig
		{
			...
			set
			{
				...
				value.Type = BarrierType;

				// Encode the seal configuration
				var buf = JsonConvert.SerializeObject((object)value); //json.Marshal(config);
        ...
        {
			get => throw new NotSupportedException("recovery not supported");
			set => throw new NotSupportedException("recovery not supported");
        }
