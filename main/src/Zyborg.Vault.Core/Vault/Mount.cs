...
namespace Zyborg.Vault
{
	public static partial class Constants
	{
		...
		public const string coreMountConfigPath = "core/mounts";
	...
	public static partial class Globals
	{
		...
		public class ErrLoadMountsFailed : Exception
		{
			public ErrLoadMountsFailed() : base("failed to setup mount table") { }
		}
		...
		//~ 	"cubbyhole/",
		//~ }
		internal static readonly IEnumerable<string> protectedMounts = new string[]
		{
			"audit/",
			"auth/",
			"sys/",
		...
		//~ 	"audit/",
		//~ }
		internal static readonly IEnumerable<string> untunableMounts = new string[]
		{
			"cubbyhole/",
			"sys/",
			"audit/",
		...
		//~ 	}
		//~ )
		internal static readonly IEnumerable<string> singletonMounts = new string[]
		{
			"cubbyhole",
			"system",
		};
	...
	public class MountTable
	{
		...
		public List<MountEntry> Entries
		{ get; set; } = new List<MountEntry>();
		...
		MountTable ShallowClone()
		{
			...
			//~ }
			//~ return mt
			var mt = new MountTable
			{
				Type = this.Type,
				Entries = new List<MountEntry>(this.Entries),
			};
			return mt;
		}
		...
		public byte[] Hash()
		{
			...
			{
				var hash = sha.ComputeHash(buf.ToUtf8Bytes());
				return hash;
			}
		}
		...
		bool setTaing(string path, bool value)
		{
			//~ n := len(t.Entries)
			//~ for i := 0; i < n; i++ {
		...
		MountTable sortEntriesByPath()
		{
			//~ sort.Slice(t.Entries, func(i, j int) bool {
			//~ 	return t.Entries[i].Path < t.Entries[j].Path
			//~ })
			//~ return t
			Entries.Sort((x, y) => x.Path.CompareTo(y.Path));
			return this;
		}
	}
	...
	public class MountEntry
	{
		...
		public bool Local
		{ get; set; }
		[JsonProperty("tainted", NullValueHandling = NullValueHandling.Ignore)]
		public bool Tainted
		{ get; set; }
		...
		public MountEntry Clone()
		{
			...
			//~ 	Tainted:     e.Tainted,
			//~ }
			var optClone = new Dictionary<string, string>(this.Options);
	...
	public class MountConfig
	{
		...
		[JsonProperty("default_least_ttl")]
		public TimeSpan DefaultLeastTTL
		{ get; set; }
		[JsonProperty("max_least_ttl")]
		public TimeSpan MaxLeaseTTL
		{ get; set; }
	}

	public static class MountCoreExtensions
	{
